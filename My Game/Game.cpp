...
void CGame::LoadImages(){  
  ...
  m_pRenderer->Load(eSprite::IdleAnimationRight, "idle_Animation_right");
  m_pRenderer->Load(eSprite::IdleAnimationLeft, "idle_Animation_left");
...
private:
    int m_nIdleFrame = 0;
    float m_fIdleFrameTimer = 0.0f;
    const float m_fIdleFrameDelay = 0.4f; // 400 ms
    int m_nIdleFrameCount = 0; // Set this to your actual frame count

    // CPlayer.cpp
    void CPlayer::UpdateIdleAnimation(float deltaTime)
    {
        m_fIdleFrameTimer += deltaTime;
        if (m_fIdleFrameTimer >= m_fIdleFrameDelay) {
            m_fIdleFrameTimer -= m_fIdleFrameDelay;
            m_nIdleFrame = (m_nIdleFrame + 1) % m_nIdleFrameCount;
        }
    }
...
void CGame::ControllerHandler(){
  ...
  if(m_pPlayer){ //safety
    ...
    if (isIdle) {
		m_pRenderer->Draw(eSprite::IdleAnimationRight, position, currentFrame);
    }
  } //if
} //ControllerHandler
    ...
    case eGameState::Waiting:
      if(m_pTimer->GetTime() - t > 3.0f){ //3 seconds has elapsed since level end
        if(m_pObjectManager->GetNumTurrets() == 0) //player won
          m_nNextLevel = (m_nNextLevel + 1)%4; //note: 4 instead of 3 FUNCTION ADDED!!
